...
class ProcurementProjectSetupSchema(BaseModel):
...
@tool
class Role:
    args_schema: Type[BaseModel] = ProcurementProjectSetupSchema
    end_flag: int = 1
        ...
        if type and project_name and project_time and subsidy_method and area:
            if not type == "查看":
                if is_ensure:
                    ...
                    result = f"<text_value>好的，收到{type}项目{project_name}命令，</text_value><confirm>请确认是否操作{type}？</confirm>"
            else:
                result = f"好的，已为您{type}项目{project_name}，项目时间：{project_time} 补贴方法：{subsidy_method} 适用区域：{area}"
        else:
            if not area:
                result = f"请输入具体的适用区域"
