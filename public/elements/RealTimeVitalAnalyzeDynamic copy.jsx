...
export default function CompactMedicalDashboard() {
  ...
  const generateECGFromCurve = useCallback((heartCurve, heartRate) => {
    const data = [];
    // heartCurve 现在是归一化的值 (-1 到 1)
    const normalizedBase = heartCurve || 0;
    const rateMultiplier = (heartRate || 75) / 75; // 根据心率调整波形频率
    
    for (let i = 0; i < 150; i++) {
      const t = i / 15 * rateMultiplier;
      // 使用归一化的曲线值作为基础波形偏移
      const baseOffset = normalizedBase * 15; // 将-1到1的范围映射到-15到15
      const value = 50 + baseOffset + 35 * (
        0.7 * Math.sin(t * 5) + 
        0.3 * Math.sin(t * 20) + 
        0.1 * Math.sin(t * 60) +
        normalizedBase * 0.3 + // 使用原始信号作为噪声基础
        (Math.random() - 0.5) * 0.2
      );
  ...
  const generateRespFromCurve = useCallback((breathCurve, breathRate) => {
    const data = [];
    // breathCurve 现在是归一化的值 (-1 到 1)
    const normalizedBase = breathCurve || 0;
    const rateMultiplier = (breathRate || 16) / 16; // 根据呼吸率调整波形频率
    
    for (let i = 0; i < 150; i++) {
      const t = i / 30 * rateMultiplier;
      // 使用归一化的曲线值作为基础波形偏移
      const baseOffset = normalizedBase * 20; // 将-1到1的范围映射到-20到20
      const value = 50 + baseOffset + 25 * (
        Math.sin(t) + 
        normalizedBase * 0.4 + // 使用原始信号作为波形调制
        ...
        <svg width="100%" height="calc(100% - 2rem)" viewBox="0 0 100 100" preserveAspectRatio="none">
          ...
          <defs>
            <pattern id={`grid-${title}`} width="10" height="10" patternUnits="userSpaceOnUse">
              <path d="M 10 0 L 0 0 0 10" fill="none" stroke={parentTheme === 'light' ? 'rgba(100, 116, 139, 0.3)' : 'rgba(100, 116, 139, 0.3)'} strokeWidth="0.5"/>
            </pattern>
          </defs>
        ...
        <svg width="100%" height="calc(100% - 2rem)" viewBox="0 0 100 100" preserveAspectRatio="none">
          ...
          <defs>
            <pattern id={`trendGrid-${title}`} width="20" height="20" patternUnits="userSpaceOnUse">
              <path d="M 20 0 L 0 0 0 20" fill="none" stroke={parentTheme === 'light' ? 'rgba(156, 163, 175, 0.4)' : 'rgba(156, 163, 175, 0.3)'} strokeWidth="0.5"/>
            </pattern>
          </defs>
