  ...
  const WaveformChart = ({ data, color, height = "200px", title }) => {
    ...
    return (
      <div className={`w-full ${themeClasses.chart} rounded border p-3 shadow-lg`} style={{ height }}>
        ...
        {/* 外部Y轴刻度标签 */}
        <div className="absolute left-1 top-8 bottom-2 pointer-events-none">
          {[-32768, -16384, 0, 16384, 32767].map((value, index) => {
            const yPercent = 10 + ((value + 32768) / 65535) * 80;
            return (
              <div 
                key={value}
                className={`absolute text-xs ${themeClasses.textMuted}`}
                style={{ 
                  top: `${100 - yPercent}%`, 
