...
export default function CompactMedicalDashboard(props = {}) {
  ...
  const [timeSpan, setTimeSpan] = useState(6); // 默认6秒
  const [showSettings, setShowSettings] = useState(false); // 控制设置菜单显示

  // 根据时间跨度计算需要保留的数据点数量
  const getDataPointsCount = (seconds) => {
    return seconds * 25; // 每秒25个数据点
  };
  ...
  const processWebSocketData = useCallback((data) => {
    ...
    if (data.type === 'realtime_data' && data.data) {
      ...
      setEcgData(prev => {
        ...
        const finalData = newData.slice(-getDataPointsCount(timeSpan));
  ...
  // WebSocket连接函数 - 改进版，防止多重连接
  const connectWebSocket = useCallback(async () => {
    // 更严格的连接状态检查，防止重复连接
    if (wsRef.current) {
      const currentState = wsRef.current.readyState;
      if (currentState === WebSocket.OPEN || currentState === WebSocket.CONNECTING) {
        console.log(`WebSocket已连接或正在连接，跳过重复连接。当前状态: ${currentState}`);
        return;
      } else {
        // 清理旧连接
  ...
  const disconnectWebSocket = useCallback(() => {
    ...
    if (wsRef.current) {
      ...
      // 只有在连接或已连接状态才主动关闭
      if (currentState === WebSocket.OPEN || currentState === WebSocket.CONNECTING) {
        wsRef.current.close(1000, '客户端主动断开'); // 使用标准关闭码
      }
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        console.log('页面重新可见');
        // 如果需要，可以在页面重新可见时重连
        if (isMonitoring && wsRef.current?.readyState !== WebSocket.OPEN) {
          setTimeout(() => connectWebSocket(), 1000); // 延迟重连
        }
      }
    };
  ...
  const WaveformChart = ({ data, color, height = "200px", title }) => {
    ...
    const points = data.map((value, index) => {
      ...
      const normalizedValue = (value + 32768) / 65535; // 0-1
      ...
      <div className={`w-full ${themeClasses.chart} rounded border p-3 shadow-lg relative`} style={{ height }}>
        ...
        <div className="ml-6 mr-2 relative" style={{ height: 'calc(100% - 4rem)' }}>
          ...
          <div className="absolute -left-6 inset-y-0 flex flex-col justify-between pointer-events-none z-10 py-0">
            {[32767, 16384, 0, -16384, -32768].map((value, index) => {
          ...
          <div className={`flex justify-between mt-1 text-xs ${themeClasses.textMuted}`}>
            <span>-{timeSpan}s</span>
            <span>-{Math.round(timeSpan * 0.75)}s</span>
            <span>-{Math.round(timeSpan * 0.5)}s</span>
            <span>-{Math.round(timeSpan * 0.25)}s</span>
            <span>0s</span>
          </div>
        </div>
      </div>
  ...
  const TrendChart = ({ data, color, height = "200px", title, fixedMin, fixedMax }) => {
    ...
    // 计算X轴时间标签 - 趋势图通常显示最近20个数据点，每个数据点约3秒间隔
    const dataPointInterval = 3; // 假设每3秒一个数据点
    const totalTimeSpan = (values.length - 1) * dataPointInterval;
    ...
    return (
      <div className={`w-full ${themeClasses.chart} rounded border p-3 shadow-lg relative`} style={{ height }}>
        ...
        <div className="ml-6 mr-2 relative" style={{ height: 'calc(100% - 4rem)' }}>
          ...
          {/* X轴刻度标签 - 显示相对时间，右侧为0 */}
          <div className={`flex justify-between mt-1 text-xs ${themeClasses.textMuted}`}>
            <span>-{totalTimeSpan}s</span>
            <span>-{Math.round(totalTimeSpan * 0.75)}s</span>
            <span>-{Math.round(totalTimeSpan * 0.5)}s</span>
            <span>-{Math.round(totalTimeSpan * 0.25)}s</span>
            <span>0s</span>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 bg-gray-700 rounded-lg p-2 border border-gray-600 shadow-lg">
        ...
        {/* 设置菜单 */}
        {showSettings && (
          <div className="absolute bottom-12 right-4 bg-gray-800 border border-gray-600 rounded-lg p-3 shadow-lg z-20">
            <div className="text-xs text-gray-300 mb-2 font-semibold">监测时间跨度</div>
            <div className="space-y-1">
              {[6, 10, 20, 30, 60].map(seconds => (
                <button
                  key={seconds}
                  onClick={() => {
                    setTimeSpan(seconds);
