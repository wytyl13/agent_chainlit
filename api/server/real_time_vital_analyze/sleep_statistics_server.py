...
class SleepStatisticsData(BaseModel):
    """睡眠统计数据模型"""
    # 设备信息
...
class SleepStatisticsServer:
    ...
    def __init__(self, sql_config_path: str):
        ...
        self.sql_provider = SqlProvider(model=SleepStatistics, sql_config_path=self.sql_config_path)
    ...
    async def get_sleep_statistics(
        ...
        try:
            ...
            if start_date or end_date:
                ...
                query = session.query(SleepStatistics)
                
                # 添加设备序列号条件
                if device_sn:
                    query = query.filter(SleepStatistics.device_sn == device_sn)
                    
                # 添加日期范围条件
                if start_date:
                    start_datetime = datetime.fromisoformat(start_date)
                    query = query.filter(SleepStatistics.sleep_start_time >= start_datetime)
                
                if end_date:
                    end_datetime = datetime.fromisoformat(end_date)
                    query = query.filter(SleepStatistics.sleep_end_time <= end_datetime)
                
                # 按睡眠开始时间降序排列
                query = query.order_by(SleepStatistics.sleep_start_time.desc())
    ...
    async def post_sleep_statistics(
        self,
        sleep_data: SleepStatisticsData,
    ):
        """
...
if __name__ == '__main__':
    ...
    async def main():
        sleep_server = SleepStatisticsServer(sql_config_path=SQL_CONFIG_PATH)
